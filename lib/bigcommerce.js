...
class BigCommerce {
  ...
  createAPIRequest() {
    ...
    const authClientHeader = this.config.clientId ? { 'X-Auth-Client': this.config.clientId } : {};
    const authHeader = this.config.useInternal ? 'X-Xsrf-Token' : 'X-Auth-Token';
    const authToken = this.config.useInternal ? this.config.xsrfToken : this.config.accessToken;
  ...
  async request(type, path, data) {
    if ((!this.config.accessToken && !this.config.xsrfToken) || !this.config.storeHash) {
      throw new Error(
    ...
    let fullPath = this.config.useInternal ?
      `/admin/services/public/stores/${this.config.storeHash}/${version}` :
      `/stores/${this.config.storeHash}/${version}`;
